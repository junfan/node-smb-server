...
FSTree.prototype.createFile = function (name, cb) {
  ...
  mkdirp(utils.getParentPath(filePath), function (err) {
    if (err) {
      ...
      //logger.error('create File:' + name + ' filePath:' + filePath)
...
FSTree.prototype.createDirectory = function (name, cb) {
  ...
  mkdirp(Path.join(this.share.path, name), function (err) {
    if (err) {
      ...
      //logger.error('create Directory:' + Path.join(self.share.path, name))
      //self.createFileInstance(Path.join(name,'.sambaprotect'), function(){})
      fs.writeFile(Path.join(self.share.path, name, '.sambaprotect'), '.', {flag:'w+'})
      self.createFileInstance(name, cb);
    }
  });
};
